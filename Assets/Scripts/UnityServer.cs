...
{
    private GameNetwork networkConnector;
    private DataSender dataSender;
    public VoltageRatioInput ratio = null;

    public const float STATUS_UPDATE_FREQ = 0.05f;
    ...
    void Start()
    {
        networkConnector = GetComponent<GameNetwork>();
        networkConnector.ConfigureAndRun(true);
        networkConnector.RegisterHandler(MsgType.Ready, StartStudy);
        dataSender = new DataSender(networkConnector);
        StartCoroutine(SendPhidgetData());
    }
    ...
    void StartStudy(NetworkMessage msg)
    {
        Debug.Log("Attempting to Connect");
    }

    public IEnumerator SendPhidgetData()
    {
        while (true)
        {
            yield return new WaitForSeconds(STATUS_UPDATE_FREQ);
            try
            {

                Debug.Log(ratio.SensorValue);
                dataSender.SendPhidgetData((float)ratio.SensorValue);
